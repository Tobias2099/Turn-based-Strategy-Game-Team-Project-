...
void randomizeplayers(Game* g, unordered_map<string, int> available, string player, int startid, int endid, char startchar) {
    ...
    for (int i = 0; i < overallcountav; i++){
        ...
        if (token[0] == 'D'){
            //add a datalink
            g->addEntityToBoard(new AbstractLink{j, x, y, c, Type::Data, player, power, false, true, 1});
            available[token] = 0;
        }

        if (token[0] == 'V'){
            //add a viruslink
            g->addEntityToBoard(new AbstractLink{j, x, y, c, Type::Virus, player, power, false, true, 1});
            available[token] = 0;
        }
        x++;
        c++;
...
bool loadplayers(Game* g, string filename, string player, int startid, int endid, char startchar){
    ...
    for (int i = startid; i <= endid; i++){
        ...
        if (token[0] == 'D'){
            //add a datalink
            g->addEntityToBoard(new AbstractLink{i, x, y, c, Type::Data, player, power, false, true, 1});
            available[token] = 0;
        }

        if (token[0] == 'V'){
            //add a viruslink
            g->addEntityToBoard(new AbstractLink{i, x, y, c, Type::Virus, player, power, false, true, 1});
            available[token] = 0;
        }
        x++;
        c++;
